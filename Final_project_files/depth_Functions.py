...
def show_img_n(image, title='', axis=None, heatmap=False, depthmap=False, figsize=(8,16)):
    if axis is None:
...
def show_array_of_images(images, split_channels=False, image_array_shape=None, 
    ...
    if (image_array_shape[0] == 1 or image_array_shape[1] == 1):
        for index, image in enumerate(images):
            show_img_n(image, axis=axis[index], title=titles[index], *args, **kwargs)
    else:
        for index, image in enumerate(images):
            show_img_n(image, axis=axis[int(index/image_array_shape[1])][int(index%image_array_shape[1])], 
